...
export default function UseDeletePosition() {
    ...
    const deletePosition = async (id) => {
        ...
        return await axios.delete(
            `https://phaseii-backend.amanahhr.com/api/admin/position/delete/${id}`,
            {
                headers: {
    ...
    return useMutation(deletePosition, {
        onSuccess: async (res, variables, context) => {
            successToaster("Position Deleted Succesfully!")
            queryClient.refetchQueries(['positions'])
        },
        onError: (err, variables, context) => {
            let errorMessage = "Error in Deleting Positions";
