...
export default function UseDeleteCompanyHandbook() {
    const queryClient = useQueryClient()

    const deleteCompanyHandbook = async (id) => {
        ...
        return await axios.delete(
            `https://phaseii-backend.amanahhr.com/api/admin/company/handbooks/delete/${id}`,
            {
                headers: {
    ...
    return useMutation(deleteCompanyHandbook, {
        onSuccess: async (res, variables, context) => {
            queryClient.refetchQueries(['companyHandbooks'])
        },
        onError: (err, variables, context) => {
            let errorMessage = "Error in deleting company handbook.";
