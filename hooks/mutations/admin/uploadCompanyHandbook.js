...
export default function UseUploadCompanyHandbook() {

    const queryClient = useQueryClient()

    const uploadCompanyHandbook = async (formData) => {     
        ...
        return await axios.post(
            `https://phaseii-backend.amanahhr.com/api/admin/company/handbooks`,
            formData,
            {
                headers: {
    ...
    return useMutation(uploadCompanyHandbook, {
        onSuccess: async (res, variables, context) => {
            queryClient.refetchQueries(['companyHandbooks'])
        },
        onError: (err, variables, context) => {
            let errorMessage = "Error in uploading company handbook";
