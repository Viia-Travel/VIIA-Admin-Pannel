...
export default function UseDeleteLocations() {
    ...
    const deletePosition = async (id) => {
        ...
        return await axios.delete(
            `https://phaseii-backend.amanahhr.com/api/admin/location/delete/${id}`,
            {
                headers: {
    ...
    return useMutation(deletePosition, {
        onSuccess: async (res, variables, context) => {
            successToaster("Location Deleted Succesfully!")
            queryClient.refetchQueries(['locations'])
        },
        onError: (err, variables, context) => {
            let errorMessage = "Error in Deleting Locations";
