...
export default function UseEditTeamAttendance() {
  const queryClient = useQueryClient()

  const teamAttendance = async (data) => {
    ...
    return await axios.post(
      `https://phaseii-backend.amanahhr.com/api/attendance/${data.id}/update`,
      data,
      {
        headers: {
  ...
  return useMutation(teamAttendance, {
    onSuccess: async (res, variables, context) => {
      queryClient.refetchQueries(['AdminAttendance'])
    },
    onError: (err, variables, context) => {
      let errorMessage = "Error in Editing Attendance";
