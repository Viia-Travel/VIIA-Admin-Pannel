...
export default function useCancelLeave() {
    const queryClient = useQueryClient();
    
    const cancelLeave = async ({ id, cancel_reason }) => {
        ...
        return await axios.post(
            `https://phaseii-backend.amanahhr.com/api/leave/cancel/${id}`,
            { cancel_reason },
            {
                headers: {
    ...
    return useMutation(cancelLeave, {
        onSuccess: async (res, variables, context) => {
            ...
            queryClient.refetchQueries(["cancelledLeaves"])
            queryClient.refetchQueries(["HRandManagerApprovalRequests"])
            successToaster("Leave Cancelled Successfully!")
        },
        onError: (err, variables, context) => {
            let errorMessage = "Error in Canceling leave. ";
