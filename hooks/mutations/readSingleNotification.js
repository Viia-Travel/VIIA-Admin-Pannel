...
export default function UseReadSingleNotification() {
  ...
  const readSingleNotification = async (id) => {
    ...
    return await axios.get(
      `https://phaseii-backend.amanahhr.com/api/read-notification/${id}`,
      {
        headers: {
  ...
  return useMutation(readSingleNotification, {
    onSuccess: async (res, variables, context) => {
      // successToaster("Notification read successfully!")
      queryClient.refetchQueries(['Notifications'])
    },
    onError: (err, variables, context) => {
      let errorMessage = "Error in marking the notification as read.";
