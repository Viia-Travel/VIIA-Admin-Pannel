...
export default function useEditLeave() {
    const queryClient = useQueryClient();

    const editLeave = async (formData) => {
        ...
        return await axios.post(
            `https://phaseii-backend.amanahhr.com/api/updateLeave`,
            formData,
            {
                headers: {
    ...
    return useMutation(editLeave, {
        onSuccess: async (res, variables, context) => {
            successToaster("Leave Updated Successfully");
        ...
        onError: (err, variables, context) => {
            let errorMessage = "Error in updating leave. ";
